...
- [ ] With an external display attached, verify the split Activity model:
  launcher Auto opens `ControlActivity` on display 0, Start Console Mode opens
...
- [x] On display 0 without external hardware, verify **Open desktop on this
  screen** and Android Back return to the control panel. The control and
  desktop Activities used distinct tasks, repeated activation focused the
  existing desktop instead of duplicating it, and no crash or ANR was logged.
