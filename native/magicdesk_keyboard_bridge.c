...
static int process_key_event(
    ...
    if (event->value == 2) {
        ...
        // Nubia reinjects external-display keys with Android repeat disabled.
        // Preserve kernel timing as a complete release/press cycle.
        struct input_event release = *event;
        release.value = 0;
...
static void enable_source_repeat(struct source_device *source) {
    if (!source->grabbed) {
        return;
    }
    const int result = magicdesk_override_source_repeat(
            source,
            KEY_REPEAT_DELAY_MS,
            KEY_REPEAT_PERIOD_MS);
    if (result <= 0) {
        fprintf(stderr,
                "MAGICDESK_KEYBOARD_REPEAT_SKIPPED path=%s error=%s\n",
                source->path,
                result == 0 ? "unsupported" : strerror(errno));
    }
}

static void enable_source_repeats(struct bridge_state *state) {
    for (int index = 0; index < state->source_count; ++index) {
        enable_source_repeat(&state->sources[index]);
    }
}
    ...
    while (!stop_requested) {
        ...
        for (int index = 0;
            ...
            if (!state->sources[index].grabbed) {
                const int grab_result = magicdesk_try_grab_source(
                        &state->sources[index]);
                if (grab_result < 0
                        && remove_source(state, index) < 0) {
                    result = -1;
                    stop_requested = 1;
                } else if (grab_result > 0) {
                    enable_source_repeat(&state->sources[index]);
                }
                break;
            }
